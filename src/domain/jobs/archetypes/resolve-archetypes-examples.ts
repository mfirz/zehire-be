...
const seniorEngineerContext: JobContext = {
  ...
  primarySignals: ["technical_depth", "decision_under_uncertainty", "system_thinking"],
...
const icuNurseContext: JobContext = {
  ...
  primarySignals: ["decision_under_uncertainty", "ethical_awareness", "accountability"],
  collaborationRequired: "high",
  customerFacing: true,
  peopleManagement: false,
...
const vpEngineeringContext: JobContext = {
  ...
  riskLevel: "high",
  decisionImpact: "business",
  primarySignals: ["stakeholder_management", "system_thinking", "accountability"],
  collaborationRequired: "high",
  customerFacing: true,
...
function runExamples() {
  ...
  if (engineerCoverage.uncoveredPrimarySignals.length > 0) {
    console.log(`  ⚠ Uncovered Primary: ${engineerCoverage.uncoveredPrimarySignals.join(", ")}`);
  }
  ...
  console.log(`  ICU Nurse: ${wouldActivate("ethical_boundary_case", icuNurseContext)}`);
  console.log(
